...
\paragraph{}{
As in Boltzmann's equations, the LBM method defines a distribution function $f(\xi, \mathbf{x}, t)$ that describes the number of particles at position $\mathbf{x}$ at time $t$ moving in direction $\xi$ at velocity $\lVert \xi \rVert_{2}$.
The parameter $\xi$ can be defined as $\xi = \frac{d\mathbf{x}}{dt}$.
...
Result of implementation can be found on ProjectPhysX/FluidX3D on github \cite{FluidX3D}
}
...
$m$ & Particle mass & $kg$ \\
$f$ & Particle distribution function (PDF) & $m^{-3} \cdot s^{-1}$ \\
$f^{eq}$ & Equilibrium distribution function & $m^{-3} \cdot s^{-1}$ \\
$\tau$ & Relaxation time & $s$ \\
$\Omega$ & Collision operator in LBM & $m^{-3} \cdot s^{-1}$ \\
$\mathbf{x}$ & Particle position & $m$ \\
$T$ & Temperature (measure of particle agitation) & $K$ \\
$R$ & Specific gas constant & $J \cdot kg^{-1} \cdot K^{-1}$ \\
$\boldsymbol{\xi}$ & Particle microscopic velocity & $m \cdot s^{-1}$ \\
\hline
\end{longtable}
\end{table}
